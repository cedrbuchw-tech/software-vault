...
export async function getMonthlyStats() {
  ...
  try {
    ...
    const { data: newUsers, error: usersError } = await supabase
      .from("profiles")
      .select("id, username, created_at, email:auth.users!inner(email)")
    ...
    // Get all programs with download counts
    const { data: programs, error: programsError } = await supabase
      .from("programs")
      .select("id, name, dl, downloads, likes");
    ...
    const downloadStats = programs
      .map((prog) => {
        ...
        if (prog.downloads && typeof prog.downloads === "object") {
          monthDownloads = prog.downloads[now.toISOString().slice(0, 7)] || 0;
        }
        return {
          id: prog.id,
          name: prog.name,
