// lib/monthly_stats.js
// Monthly statistics gathering and email template
 ...
 *
 * The scheduled report runs at 01:00 on the 1st, so it asks for -1. "This
 * month" is about an hour old at that point and always empty, which made the
 * report worthless even on the occasions it did go out.
 */
export async function getMonthlyStats(monthOffset = 0) {
  ...
  try {
    ...
    //
    // This used to select `email:auth.users!inner(email)`. PostgREST only serves
    // the schemas Supabase exposes and `auth` is not one of them, so that embed
    // failed every single time — getMonthlyStats threw before it reached
    // anything else, and no report could ever be generated.
    ...
    // Addresses live in auth.users, which only the admin API can read. Best
    // effort: a report with names but no addresses still beats no report.
    ...
    // Programs and their counters.
    //
    // This used to ask for `downloads`, which is the per-OS build links
    // (win/mac/lin), not download counts — and on a database where that column
    // is missing entirely the request failed and killed the whole report. Ask
    // for the useful set, and drop back to the columns every version of this
    // schema has if the database refuses, so one absent column can never take
    // the report down again.
    ...
    // Calculate download changes for this month (if downloads field is JSON)
    const downloadStats = programs
      .map((prog) => {
        // dl_by_month is written by increment_program_downloads (see
        // MIGRATION_PROGRAM_STATS.sql) and looks like {"2026-08": 12}. It only
        // has figures from the day that migration was run onwards; before that
        // the site kept a lifetime total and nothing else, so older months
        // legitimately read 0.
      ...
      // lifetime total breaks the tie, so the list still ranks sensibly in the
      // months before per-month counting was switched on
      .sort((a, b) => (b.monthDownloads - a.monthDownloads)
                   || (b.totalDownloads - a.totalDownloads));

    // head:true asks for the number only, instead of downloading every row to count it
